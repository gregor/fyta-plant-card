...
class FytaPlantCard extends LitElement {
  ...
  getCardSize() {
    ...
    const headerSize = this.config?.display_mode === DisplayMode.FULL ? 100 : 90; // Base size
    ...
    const attributesSize = Math.ceil(sensorCount / 2) * 30 + (this.config?.display_mode === DisplayMode.FULL ? 30 : 0);

    const cardSize = (headerSize + attributesSize) / 50;
